...
export const Route = createFileRoute("/analyse/pgn")({
  component: AnalysePgnPage,
});
...
function AnalysePgnPage() {
  ...
  const handleAnalyse = async () => {
    ...
    setResult(null);
    try {
      const res = await apiAnalysePgn(pgnInput);
      setResult(res);
      setCurrentMoveIndex(-1);
      setExplorationMoves([]);
      setEvalResult(null);
    } catch (err: any) {
      setError(err.message || "Failed to analyse PGN");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (status !== "authenticated" || !user) {
    return (
      <div style={centerPage}>
        ...
        <p style={{ color: "#6B7264", marginBottom: 16 }}>Sign in to analyse PGNs</p>
  ...
  if (!result) {
    return (
      <div style={pageStyle}>
        ...
        <div style={{ position:"relative", zIndex:1, width:"100%", maxWidth:600, padding: "40px 20px", margin:"0 auto", animation:"fadeIn .5s ease", display: "flex", flexDirection: "column" }}>
          <h1 style={{ fontSize: "2rem", fontWeight: 800, color: "#1A1A1A", marginBottom: 8, textAlign: "center" }}>PGN Analysis</h1>
          <p style={{ color: "#6B7264", fontSize: ".9rem", marginBottom: 32, textAlign: "center" }}>Paste your PGN below to analyse it with Stockfish.</p>

          {error && <div style={{ padding: "12px", background: "rgba(239,68,68,.1)", color: "#EF4444", borderRadius: 8, marginBottom: 16, fontSize: ".9rem", textAlign: "center" }}>{error}</div>}
          ...
          <textarea
            ...
            style={{
              ...
              resize: "vertical", outline: "none", boxShadow: "inset 0 2px 4px rgba(0,0,0,.02)"
            }}
          />
          ...
          <button
            ...
            disabled={loading}
            style={{
              marginTop: 20, padding: "14px 24px", borderRadius: 8,
              background: P, color: "#fff", fontWeight: 700, fontSize: "1rem",
  ...
  const whitePlayer = result.metadata?.white || "White";
  const blackPlayer = result.metadata?.black || "Black";
  const pgnResultStr = result.metadata?.result || "*";
  ...
  return (
    <div style={pageStyle}>
      ...
      <nav style={{ width:"100%", padding: isMobile ? "10px 14px" : "10px 28px", display:"flex", alignItems:"center", justifyContent:"space-between", background:"rgba(250,250,247,.88)", backdropFilter:"blur(16px)", borderBottom:"1px solid rgba(139,115,85,.06)", flexShrink:0, position:"sticky", top:0, zIndex:100 }}>
        ...
        <button onClick={() => { setResult(null); setPgnInput(""); }} style={{ padding:"5px 12px", borderRadius:6, border:"1px solid rgba(139,115,85,.1)", background:"transparent", cursor:"pointer", color:"#6B7264", fontSize:".78rem", fontWeight:500 }}>
          + New PGN
        </button>
      </nav>
      ...
      <div style={{ position:"relative", zIndex:1, width:"100%", maxWidth:1200, padding: isMobile ? "12px 8px" : "24px 24px", margin:"0 auto", animation:"fadeIn .5s ease" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16, flexWrap:"wrap", gap:10 }}>
          <div>
            <h1 style={{ fontSize: isMobile ? "1.3rem" : "1.6rem", fontWeight:800, color:"#1A1A1A", margin:0 }}>PGN Analysis</h1>
            <p style={{ color:"#8B9080", fontSize:".78rem", marginTop:4 }}>
              {whitePlayer} vs {blackPlayer} {result.metadata?.event ? `· ${result.metadata.event}` : ""} {result.metadata?.date ? `· ${result.metadata.date}` : ""}
            </p>
          </div>
          {pgnResultStr && pgnResultStr !== "*" && (

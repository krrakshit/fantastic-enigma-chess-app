  ...
  if (status === "loading" || loading) {
    return (
      <div style={centerPage}>
        <div style={{ width: 36, height: 36, border: "2px solid #1F2937", borderTop: `2px solid ${P}`, borderRadius: "50%", animation: "spin .8s linear infinite" }} />
        <p style={{ color: "#6B7280", marginTop: 16 }}>Loading history...</p>
      </div>
    );
  }
  ...
  if (status !== "authenticated" || !user) {
    return (
      <div style={centerPage}>
        ...
        <p style={{ color: "#6B7280", marginBottom: 16 }}>Sign in to view your game history</p>
  ...
  return (
    <div style={pageStyle}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 820, width: "100%", animation: "fadeIn .5s ease" }}>
        <Link to="/" style={{ color: "#4B5563", textDecoration: "none", fontSize: ".82rem", display: "inline-flex", alignItems: "center", gap: 6, marginBottom: 24 }}
          onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.color = P; }}
          onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.color = "#4B5563"; }}
        >← Home</Link>

        <h1 style={{ fontSize: "2rem", fontWeight: 800, color: "#E5E7EB", marginBottom: 8 }}>Game History</h1>
        <p style={{ color: "#6B7280", fontSize: ".9rem", marginBottom: 32 }}>Your matches, results, and analysis.</p>

        {/* Stats grid */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 36 }}>
          {[
            { label: "Games", value: totalGames, color: "#E5E7EB" },
            ...
            <div key={i} style={statCard}>
              ...
              <div style={{ fontSize: ".68rem", color: "#4B5563", letterSpacing: ".08em", fontWeight: 600, marginTop: 2 }}>{s.label.toUpperCase()}</div>
            </div>
          ))}
        </div>
        ...
        {/* Games list */}
        {games.length === 0 ? (
          <div style={{ textAlign: "center", padding: 60, background: "rgba(255,255,255,.02)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 16 }}>
            <div style={{ fontSize: "3rem", marginBottom: 14, opacity: 0.4 }}>♟</div>
            <p style={{ color: "#6B7280" }}>No games yet. Go play!</p>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: ".9rem", fontWeight: 600, color: "#E5E7EB" }}>
                      vs <span style={{ color: P }}>{opponentId}</span>
                    </div>
                    <div style={{ fontSize: ".72rem", color: "#4B5563", marginTop: 2 }}>
                  ...
                  {g.status === "finished" && (
                    <Link to="/analyse/$roomId" params={{ roomId: g.roomID }} style={analyseBtn}
                      onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = "rgba(16,185,129,.15)"; }}
